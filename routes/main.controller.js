...
let register = async(req,res)=>{
    ...
    let sessionID = []
    const insert = `insert into users (userID, password ,email, regiTime, useSet, sessionID ,timestamp) values(?,?,?,?,?,?,?)`;
    const select = `SELECT userID FROM users WHERE userID= ?`;
    const hashPassword = await bcrypt.hash(password,12);
    const useSet = [{"type" : "ottGroups","setting" : ["최근 추가된 OTT 콘텐츠"]},{"type" : "channelGroups","setting" : ["최근 추가된 채널"]},{"type" : "youvidGroups","setting" : ["최근 추가된 유튜브 영상"]},{"type" : "streamerGroups","setting" : ["최근 추가된 스트리머"]},{"type" : "darkmode","setting" : 0},{"type" : "subscription","setting" : "true,true,true,true"}];
    ...
    if(req.session.user){
        ...
        .then(rows=>{
            if(rows.length !== 0){
                ...
                db.putData(insert,[userid,hashPassword,email,now,`${JSON.stringify(useSet)}`,`${JSON.stringify(sessionID)}`,now])
                .then(p=>{
                    res.status(200).json({   
                        "content_type" : "json" ,
                        "result_code" : 200 ,
...
let userBehaviorGet = async(req,res)=>{
    ...
    let query = `SELECT * FROM user_behavior WHERE userID= ? and timestamp >= ?`;
...
let searchGet = async (req,res)=>{
    ...
    let query = `SELECT title,rawtitle,jwURL,jwimg FROM specification WHERE title like ? or rawtitle like ?`
    if(req.session.user){
        try{
...
let recommend = async (req,res)=>{
    ...
    let year = today.getFullYear();
    let now = `${year}-${month}-${day} ${today.getHours()}:${today.getMinutes()}:${today.getSeconds()}`;
    let specSearch = await db.getData(`select rawtitle from specification where title like ? and Offers like ?`,[`%${title}%`,`%${platform}%`])
    let type;
